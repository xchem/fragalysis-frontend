...
export const QualityStatusModal = memo(
  ({ openModal, onModalClose, statuses, latestPeerReviews, site_observation, anchorElQualityStatus }) => {
          ...
          <Grid container justifyContent="flex-start" direction="column" spacing={2}>
            <Grid item container direction="row" justifyContent="space-between" alignItems="center" spacing={2}>
              <Grid className={classes.gridItem} item xs={2}>
                <QualityStatusLight
                  size={18}
                  status={getMainQualityStatusObject()?.status}
                  props={{ className: classes.mainStatusLight }}
                />
              </Grid>
              {DJANGO_CONTEXT.pk && (
                <>
                  ...
                  <RichTooltip path="setMainStatus">
                    <Grid className={classes.gridItem} item xs={1}>
                      <IconButton
                        size="small"
                        onClick={addMainStatusHandler}
                        disabled={selectedStatus === getMainQualityStatusObject()?.status}
                      >
                        <Check />
                      </IconButton>
                    </Grid>
            ...
            <Grid item container direction="row" justifyContent="flex-start" alignItems="center" spacing={2}>
              ...
              {DJANGO_CONTEXT.pk && (
                <Grid className={classes.gridItem} item xs>
                  ...
                  <PeerReviewModal
                    openDialog={Boolean(anchorEl)}
                    onDialogClose={() => setAnchorEl(null)}
                    anchorEl={anchorEl}
            ...
            <Table>
              <TableBody>
                {statuses?.map((status, index) => {
                  return (
                    <TableRow key={index} className={classNames({ [classes.mainStatusRow]: status.main_status })}>
                      <TableCell className={classNames(classes.posePropertiesTableCell, classes.lightsTableCell)}>
                        <QualityStatusLight
                          size={15}
                          status={status.previous_status?.status || QUALITY_STATUSES.NONE}
                        />
                        {'➜'}
                        <QualityStatusLight size={15} status={status.status} />
                      </TableCell>
                      <TableCell className={classes.posePropertiesTableCell}>
                        {status.user ? `${status.first_name} ${status.last_name}` : ''}
                      </TableCell>
                      <TableCell className={classes.posePropertiesTableCell}>
                        {getDateDifference(status.timestamp)}
                      </TableCell>
                      <RichTooltip path="comment" values={{ comment: status.comment }}>
                        <TableCell className={classNames(classes.posePropertiesTableCell, classes.commentTd)}>
                          {status.comment}
                        </TableCell>
                      </RichTooltip>
                    </TableRow>
