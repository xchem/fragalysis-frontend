...
export const NumericView = memo(({ column, data }) => {
  ...
  const activityDataSet = useMemo(() => {
    return (
      data?.activityData
        ?.filter(activity => activity.property_name === column.name)
        .map(activity => getDataValue(activity, column.data_type)) || []
    );
  ...
  const activityCellData = useMemo(() => {
    return activityDataSet.length > 0
      ? activityDataSet.length > 1
        ? `${(activityDataSet.reduce((acc, curr) => acc + curr, 0) / activityDataSet.length).toFixed(2)} (n=${
            activityDataSet.length
          })`
        : activityDataSet[0]
      : '';
  }, [activityDataSet]);
  ...
  return (
    <Grid container direction="row" justifyContent="center" alignItems="center" wrap="nowrap">
      <RichTooltip path="numericValue" values={{ value: activityTitleData }}>
