...
export const EditTagsModal = memo(({ open, anchorEl, setOpenDialog }) => {
  ...
  const compoundsCount = useMemo(() => {
    ...
    if (tag && tag['site_observations']) {
      let uniqueCompounds = [];
      allMolList
        .filter(molecule => tag.site_observations.includes(molecule.id))
        .forEach(observation => {
        ...
        <Grid container direction="column">
                ...
                {tags?.map(tag => (
                  <MenuItem key={`tag-editor-new-category-${tag.id}`} value={tag.id}>
                    <Grid container alignItems="center" spacing={1}>
                      <ColorPicker selectedColor={getColourForTag(tag)} disabled={true} />
          ...
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            ...
            {isRestrictedCategory
              ? rightSide(<TextField disabled={true} value={getTagCategory(tag)} />)
              : rightSide(
                  <Select
          ...
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            ...
            {rightSide(
              <Grid item container direction="row" alignItems="center">
                <Grid item xs>
                  <TextField disabled={true} value={tag?.tag_prefix || ''} />
                </Grid>
                <Grid item xs className={classes.leftSide}>
                  <InputLabel>Upload name</InputLabel>
                </Grid>
                <Grid item xs>
                  <RichTooltip path="uploadName" values={{ uploadName: tag?.upload_name || '' }}>
                    <TextField disabled={true} value={tag?.upload_name || ''} />
                  </RichTooltip>
                </Grid>
              </Grid>
            )}
          ...
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            ...
            {rightSide(<TextField onChange={onNameForNewTagChange} value={newTagName} placeholder="Name" />)}
          </Grid>
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            {leftSide('Colour')}
            {rightSide(
          ...
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            {leftSide('#Compounds')}
            {rightSide(<TextField disabled={true} value={compoundsCount} />)}
          </Grid>
          <Grid item container direction="row" alignItems="center" className={classes.row}>
            {leftSide('#Observations')}
            {rightSide(<TextField disabled={true} value={tag?.site_observations?.length || 0} />)}
          </Grid>
          {/* Buttons */}
