...
export const saveAndShareSnapshot = (target = undefined) => (dispatch, getState) => {
  ...
  if (targetId && target) {
    ...
    dispatch(createProjectFromSnapshotDialog(data))
      .then(() => {
        const userEmail = DJANGO_CONTEXT['email'];
        const title = moment().format('-- YYYY-MM-DD -- HH:mm:ss');
        const description =
          loggedInUserID === undefined ? 'Snapshot generated by anonymous user' : `snapshot generated by ${userEmail}`;
        const type = SnapshotType.MANUAL;
        const author = loggedInUserID || null;
        const parent = null; // isForceProjectCreated === false ? currentSnapshot.id : null;
        const session_project = getState().projectReducers.currentProject.projectID;

        return dispatch(createNewSnapshot({ title, description, type, author, parent, session_project }));
      })
      .catch(error => {
        throw new Error(error);
      });
