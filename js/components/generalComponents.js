...
export class GenericList extends React.PureComponent {
  ...
  getUrl() {
    ...
    if (this.list_type == listTypes.TARGET) {
      ...
      if (
        this.props.target_on != undefined &&
        this.props.mol_group_on != undefined
      ) {
        // mol group choice
        base_url += "molecules/";
      ...
      if (
        this.props.target_on != undefined &&
        this.props.pandda_site_on != undefined
      ) {
        // mol group choice
        base_url += "events/";
        get_params.target_id = this.props.target_on;
  ...
  processResults(json) {
    ...
    if (
      this.list_type == listTypes.SESSIONS &&
      this.props.seshListSaving == true
    ) {
  ...
  loadFromServer() {
    ...
    if (url.toString() != this.old_url) {
      ...
      fetchWithMemoize(url).then(json => {
        ...
        if (
          this.list_type === listTypes.MOLECULE &&
          mol_group_on &&
          setCachedMolLists
        ) {
...
export class GenericView extends React.PureComponent {
    ...
    if (this.key == undefined) {
      if (url.toString() != this.old_url) {
        fetchWithMemoize(url).then(text =>
          this.setState(prevState => ({ img_data: text }))
        );
      }
    } else {
      if (url.toString() != this.old_url) {
        fetchWithMemoize(url).then(text =>
          this.setState(prevState => ({ img_data: text[this.key] }))
        );
      }
    }
    this.old_url = url.toString();
  ...
  render() {
    ...
    this.current_style = this.state.isToggleOn
      ? this.selected_style
      : this.not_selected_style;
    return (
...
export class Slider extends React.PureComponent {
  ...
  render() {
    ...
    var meat_of_div;
    if (
      this.props.object_list == undefined ||
  ...
  handleChange(selected) {
    ...
    var prog_string =
      ...
      (selected + 1).toString() +
      " of a total of " +
      this.props.object_list.length.toString();
